...
func TestDefaults(t *testing.T) {
	cfg := &Config{}
	_, err := config.Defaults(cfg, defaultsData)
...
func TestSave(t *testing.T) {
	cfg1 := &Config{}
	_, err := config.Load(cfg1, "testdata/config_valid.toml", defaultsData, true)
	require.NoError(t, err)
	cfgPath := filepath.Join(t.TempDir(), "config.toml")
	err = config.Save(cfg1, cfgPath, 0600)
	require.NoError(t, err)
	cfg2 := &Config{}
	_, err = config.Load(cfg2, cfgPath, defaultsData, true)
	require.NoError(t, err)
...
func TestLoadValid(t *testing.T) {
	cfg := &Config{}
	_, err := config.Load(cfg, "testdata/config_valid.toml", defaultsData, true)
	require.NoError(t, err)
}

func TestLoadInvalidDuration(t *testing.T) {
	cfg := &Config{}
	_, err := config.Load(cfg, "testdata/config_invalid_duration.toml", defaultsData, true)
	require.Error(t, err)
}

func TestLoadInvalidNetworks(t *testing.T) {
	cfg := &Config{}
	_, err := config.Load(cfg, "testdata/config_invalid_networks.toml", defaultsData, true)
	require.Error(t, err)
}

func TestLoadInvalidRegexp(t *testing.T) {
	cfg := &Config{}
	_, err := config.Load(cfg, "testdata/config_invalid_regexp.toml", defaultsData, true)
	require.Error(t, err)
}

func TestLoadInvalidTimeLocation(t *testing.T) {
	cfg := &Config{}
	_, err := config.Load(cfg, "testdata/config_invalid_time_location.toml", defaultsData, true)
	require.Error(t, err)
}

func TestLoadInvalidURLs(t *testing.T) {
	cfg := &Config{}
	_, err := config.Load(cfg, "testdata/config_invalid_urls.toml", defaultsData, true)
	require.Error(t, err)
}

func TestLoadInvalidEnum(t *testing.T) {
	cfg := &Config{}
	_, err := config.Load(cfg, "testdata/config_invalid_enum.toml", defaultsData, true)
	require.Error(t, err)
}

func TestLoadInvalidUnknownField(t *testing.T) {
	cfg := &Config{}
	_, err := config.Load(cfg, "testdata/config_unknown_field.toml", defaultsData, true)
	require.Error(t, err)
}

func TestLoadNonStrict(t *testing.T) {
	cfg := &Config{}
	_, err := config.Load(cfg, "testdata/config_unknown_field.toml", defaultsData, false)
	require.NoError(t, err)
}
